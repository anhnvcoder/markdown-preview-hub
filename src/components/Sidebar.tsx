...
import { uploadFiles, uploadFolder } from '../stores/file-store';
import { showUploadWarning } from './UploadWarningModal';

// Shared signals for inline creating - accessible by FileItem
export const inlineCreating = signal<{ type: 'file' | 'folder'; parentId: string | null } | null>(null);
    ...
    if (selectedNodeId.value) {
      ...
      if (selected) {
        if (selected.type === 'folder') {
          ...
          if (parentPath) {
            const parentFolder = files.value.find(f => f.path === parentPath && f.type === 'folder');
      ...
      >
        {!project ? (
          <div class="text-sm text-muted-foreground p-2 flex flex-col items-center justify-center h-full text-center">
            ...
            <button class="btn-primary text-xs gap-1.5" onClick={() => openFolder()}>
